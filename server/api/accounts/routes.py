...
async def signup(request: Request, response: Response):
    ...
    user_data = UserCreate(**data)  # ty:ignore[invalid-argument-type]
...
async def login(request: Request, response: Response):
    ...
    data = await request.json
    login_data = UserLogin(**data)  # ty:ignore[invalid-argument-type]
...
async def refresh_token(request: Request, response: Response):
    ...
    return {"user": user_response, "message": "Token refreshed successfully"}
...
@router.get(
    "/admin/users", summary="Get all users (Admin only)", responses=UsersListResponse
)
...
async def admin_create_user(request: Request, response: Response):
    ...
    user = cast(Account, request.user)  # Check if user is admin
    if not user.is_authenticated or user.role != Role.ADMIN:
        return response.json(
            {"detail": "Admin access required"}, status_code=status.HTTP_403_FORBIDDEN
        )
    ...
    data = await request.json
    user_data = AdminUserCreate(**data)  # ty:ignore[invalid-argument-type]
    ...
    return response.json(user_response.dict(), status_code=status.HTTP_201_CREATED)
...
async def admin_update_user(request: Request, response: Response, user_id: str):
    ...
    data = await request.json
    update_data = UserUpdate(**data)  # ty:ignore[invalid-argument-type]
...
async def admin_delete_user(request: Request, response: Response, user_id: str):
    ...
    return response.json(
        {"message": "User deleted successfully"}, status_code=status.HTTP_204_NO_CONTENT
    )
