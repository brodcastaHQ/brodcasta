...
async def poll_connect(request: Request, response: Response, project_id: str):
...
async def poll_send(request: Request, response: Response, project_id: str):
    ...
    try:
        ...
        if not somal_middleware.is_channel_authenticated(channel):
            return response.json(
                {"error": "Channel not authenticated with SOMAL"}, status_code=403
            )
        ...
        if room_name:
            ...
            if project.auth_type == AuthType.PUBLISHING_ONLY and not token:
                return response.json(
                    {"error": "Token required for publishing"}, status_code=403
                )

            if not await somal_middleware.check_room_permission(
                channel, room_name, "publish"
            ):
                return response.json(
                    {"error": f"Permission denied for room: {room_name}"},
                    status_code=403,
                )
...
async def get_poll_stats(request: Request, response: Response, project_id: str):
